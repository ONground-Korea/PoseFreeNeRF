...
class SynthDataset(Dataset):
    ...
    #            theta = theta[[0,1,4,2,3,5]].reshape(2,3)
            ...
            if self.geometric_model=='affine' or self.geometric_model=='afftps':
                rot_angle = (np.random.rand(1)-0.5)*2*np.pi/12; # between -np.pi/12 and np.pi/12 15도 rotation
                sh_angle = (np.random.rand(1)-0.5)*2*np.pi/6; # between -np.pi/6 and np.pi/6 30도 shear
                lambda_1 = 1+(2*np.random.rand(1)-1)*0.25; # between 0.75 and 1.25
                lambda_2 = 1+(2*np.random.rand(1)-1)*0.25; # between 0.75 and 1.25
                tx=(2*np.random.rand(1)-1)*0.25;  # between -0.25 and 0.25 평행이동
